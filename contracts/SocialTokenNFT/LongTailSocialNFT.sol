...
contract LongTailSocialNFT is ISocialTokenNFT, IAuxCompatableNFT, ERC721, SizeSortedList {
    ...
    function tokenAuxURI(uint256 tokenId, bytes memory signedMessage) public view virtual returns(bool isDifferent, string memory uri) {
        ...
        return (true, string(abi.encodePacked(auxTokenURIs[groupData[tokenData.group][tokenData.level - 1].uriIndex], 
            _toHex(keccak256(abi.encode(tokenData))))));
    }
    ...
    function _toHex(bytes32 _bytes32) private pure returns (string memory) {
        uint8 _f;
        uint8 _l;
